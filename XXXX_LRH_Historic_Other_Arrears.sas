...
create table temp1 as 
	...
	from SQLSERV.XXXX_LRH_Historic_Other_Acc_SAS
...
/*################ Deletes existing table on server and uploads the new one ##############*/
  %macro delds;
     %if %sysfunc(exist(SQLSERV.XXXX_LRH_Historic_Other_Arrears)) %then %do;
       proc datasets library=SQLSERV;
       delete XXXX_LRH_Historic_Other_Arrears;
      run;
     %end; 
      %mend;
      %delds;
...
DATA    SQLSERV.XXXX_LRH_Historic_Other_Arrears;
SET     acc;
RUN;
